...
describe('NFT', () => {
  ...
  describe('Deployment', () => {
    ...
    it('returns the cost to mint a token', async () => {
  ...
  describe('Minting', () => {
    ...
    describe("Success", async () => {
      ...
      it('returns IPFS URI of the token', async () => {
      ...
      it('updates the contract ether balance', async () => {
        expect(await ethers.provider.getBalance(nft.address)).to.equal(ether(10))
      })
    ...
    describe("Failure", async () => {
      it('rejects insufficient payment', async () => {
        ...
        await expect(nft.connect(minter).mint(1, {value: ether(1)})).to.be.revertedWith('Not enough payment')
      })
      ...
      it('rejects at least 1 NFT to be minted', async () => {
        ...
        await expect(nft.connect(minter).mint(0, { value: COST })).to.be.revertedWith('Must mint at least one token')
      })
      ...
      it('dose not allow more NFTs to be minted than max amount', async () => {
        const ALLOW_MINTING_ON = Date.now().toString().slice(0, 10) // Now
        const NFT = await ethers.getContractFactory('NFT')
